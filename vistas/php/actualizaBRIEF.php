<?php 
    ...
    $sql="UPDATE briefs SET fechaSalida='$fechaF', fechaAnterior='$fechaAnterior' WHERE id='$id'";
    ...
    if($result){
        ...
        $sql="SELECT * from briefs WHERE id='$id'";
		...
		$correoEjecutiva=$consulta['ejecutiva'];
		}
	...
	$sql="SELECT correo from equipoDireccion ";
     ...
     function enviarCorreo($correos,$proyecto,$fechaEntrega,$fechaAnterior){
		...
		$mailsubject = '¡Cambio de Fecha BRIEF'.' - '. $proyecto .'!';
		$firstname = '';
		$lastname = '';
		$description = '';
