	...
	if(count($arrayE)==0){
		...
		if (!mysqli_query($conexion,$sql)){
			...
			for ($x=0;$x<count($personas); $x++){
			    ...
			    $query_arte = $mysqli -> query("SELECT DISTINCT nombre FROM equipoArte");
				while ($valores_arte = mysqli_fetch_array($query_arte)) {
					$equipoArte[] = utf8_encode($valores_arte[0]);
				}
				$query_post = $mysqli -> query("SELECT DISTINCT nombre FROM equipoPostProduccion");
				while ($valores_post = mysqli_fetch_array($query_post)) {
					$postphp[] = utf8_encode($valores_post[0]);
				}
				$query_estrategia = $mysqli -> query("SELECT DISTINCT nombre FROM equipoEstrategia");
				while ($valores_estrategia = mysqli_fetch_array($query_estrategia)) {
					$estrategiaphp[] = utf8_encode($valores_estrategia[0]);
				}
				$query_ilustracion = $mysqli -> query("SELECT DISTINCT nombre FROM equipoIlustracion");
				while ($valores_ilustracion = mysqli_fetch_array($query_ilustracion)) {
					$ilustracionphp[] = utf8_encode($valores_ilustracion[0]);
				}
				$query_copy = $mysqli -> query("SELECT DISTINCT nombre FROM equipoCopyCorreccion");
				while ($valores_copy = mysqli_fetch_array($query_copy)) {
					$copyphp[] = utf8_encode($valores_copy[0]);
				}
				$query_programacion = $mysqli -> query("SELECT DISTINCT nombre FROM equipoProgramacion");
				while ($valores_programacion = mysqli_fetch_array($query_programacion)) {
					$programacionphp[] = utf8_encode($valores_programacion[0]);
				}
				$query_admin = $mysqli -> query("SELECT DISTINCT nombre FROM equipoAdministracion");
				while ($valores_admin = mysqli_fetch_array($query_admin)) {
					$adminphp[] = utf8_encode($valores_admin[0]);
				}



			    if (in_array($cadena, $equipoArte)) {
					$correo_arte = $mysqli -> query("SELECT DISTINCT correo FROM equipoArte WHERE nombre LIKE '%$nombres_sa%'");
					...
					$correo_post = $mysqli -> query("SELECT DISTINCT correo FROM equipoPostProduccion WHERE nombre LIKE '%$nombres_sa%'");
					...
					$correo_estrategia = $mysqli -> query("SELECT DISTINCT correo FROM equipoEstrategia WHERE nombre LIKE '%$nombres_sa%'");
					...
					$correo_ilustracion = $mysqli -> query("SELECT DISTINCT correo FROM equipoIlustracion WHERE nombre LIKE '%$nombres_sa%'");
					...
					$correo_copy = $mysqli -> query("SELECT DISTINCT correo FROM equipoCopyCorreccion WHERE nombre LIKE '%$nombres_sa%'");
					...
					$correo_programacion = $mysqli -> query("SELECT DISTINCT correo FROM equipoProgramacion WHERE nombre LIKE '%$nombres_sa%'");
					...
					$correo_admin = $mysqli -> query("SELECT DISTINCT correo FROM equipoAdministracion WHERE nombre LIKE '%$nombres_sa%'");
         ...
         $handle = fopen($ruta, "r");  // set the file handle only for reading the file
         $content = fread($handle, $tam_archivo); // reading the file
         fclose($handle);
...
/////////////////SE ENVIA EMAIL A CLIENTE//////////////////////////////////
          ...
          $encoded_content = chunk_split(base64_encode($archivos)); //CAMBIO
