...
if($tabla == "odt"){
    ...
    //$Nombre_Proyecto = utf8_encode($Nombre_Proyect);
    $Nombre_Pieza = $id . '-' . $subject2 . '-ODT';
    //echo " nom " . ($Nombre_Pieza);
} else{
    //echo "tabla brief";
    $subject = $_GET['nom_proy'];
    $subject2 = $_GET['nom_pieza'];
...
<script type="text/javascript">
    ...
    function act_comentario(id,valor) {
        var valC=valor;
        var ip=id;
        actualizarSemaforoNew(valC, "comentarios", ip);
    }
</script>
</script>
</head>
...
<div class="container-fluid table-wrapper" style="background-color: #000000;">
    ...
    if (empty($subject2) == false){
        //echo "Funciona";
        $fechas_dif = $mysqli -> query("SELECT fechaInicio, fechaSalida FROM piezas WHERE nombre = '$subject2' AND id = '$id'");
        ...
        $fecha_inicial_dia = substr($fecha_inicial, 8, 2);
        //echo $fecha_inicial_dia;

        if($fecha_inicial_dia == "31"){
            //echo " HEY";
            $cap_fecha_inicial = substr($fecha_inicial, 0, 10);
            $fecha_inicial2 = $fecha_inicial;

            $resultado = str_replace("31", "30", $cap_fecha_inicial);

            //echo "resultado" . $resultado;



            $fecha_inicial = $resultado;
            
            $date1  = $fecha_inicial;
            $date2  = $fecha_final;
            $output = [];
            $time   = strtotime($date1);
            //echo " valor de time " . $time;
            ...
            //echo " mes y year " . implode(",", $output);
            $reset = 1;
        ...
        $fechas_dif = $mysqli -> query("SELECT idOdt, fechaSolicitud, fechaEntrega FROM odts WHERE proyecto = '$subject' AND id = '$id'");
        while($fechas_proy = mysqli_fetch_array($fechas_dif)) {
            $fecha_inicial = $fechas_proy[fechaSolicitud];
            $fecha_final = $fechas_proy[fechaEntrega];
    ...
    <div class="container-fluid" id="tablatocsv">
        <div id="tabla">
            <div class="row">
                ...
                <?php
                    if(empty($subject2) == false){
                    	?><h1 class="text-center"><span class="">Tabla de Tiempos: <?php echo utf8_encode($subject) ?> - <?php echo utf8_encode($subject2) ?></span></h1><?php
                    }else{
                    	?><h1 class="text-center"><span class="">Tabla de Tiempos: <?php echo utf8_encode($subject) ?></span></h1><?php
                    }
                ?>
                    ...
                    <table class="table table-condensed table-bordered" id="table_tt">
                        <caption>
                            <button class="btn agregar_nuevo" data-toggle="modal" id="agregar_nuevo">
                                Agregar Nueva Tarea
                            ...
                            <?php
                                ...
                                <button class="btn agregar_pieza" data-toggle="modal" id="agregar_pieza">
                                Agregar Nueva Pieza
                                <span class="glyphicon glyphicon-plus"></span>
                            </button>   
                            <?php
                        ...
                        <tr>
                            ...
                            <?php
                            if (empty($subject2) == false){
                                $sql_semaforo = $mysqli -> query("SELECT id,semaforo FROM piezas WHERE nombre = '$subject2'");
                                while($colores_semaforo = mysqli_fetch_array($sql_semaforo)) {
                                    $color_semaforo = $colores_semaforo[semaforo];
                                    $a=$colores_semaforo[id];
                                }
				...
				echo '<td> 
					<div class="radio-item">';
					    if( $color_semaforo=="#FFC500" ){
					        ...
					        echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="InicioB" name="droneB" checked value="#FFC500">';
					    }else{
					        echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="InicioB" name="droneB" value="#FFC500">';
					    }
					    echo '<label id="iniciolabel" for="InicioB"></label>
					</div>
					<div class="radio-item">';
					    if($color_semaforo=="#1CB707"){
					        echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="Enviado1" name="droneB" checked value="#1CB707">';
					    }else{
					        echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="Enviado1" name="droneB" value="#1CB707">';
					    }
					...
					if($color_semaforo=="#FF0C00"){
					    echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="PausadoB" name="drone" checked value="#FF0C00">';
					}else{
					    echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="PausadoB" name="drone" value="#FF0C00">';
					}
					...
					<div class="radio-item">';
					    if($color_semaforo=="#6900D1"){
					        echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="CerradoB" name="droneB" checked value="#6900D1">';
					    }else{
					        echo '<input onclick="act_semaforo_p('.$a.',value)" type="radio" id="CerradoB" name="droneB" value="#6900D1">';
					    }
					    ...
					    echo '<label id="cerradolabel" for="CerradoB"></label>
                                ...
                                $sql_semaforo = $mysqli -> query("SELECT id,semaforo FROM odts WHERE idOdt = '$idodt'");
                                while($colores_semaforo = mysqli_fetch_array($sql_semaforo)) {
                                    $color_semaforo = $colores_semaforo[semaforo];
                                    $ap=$colores_semaforo[id];
                                }
				...
				echo '<td> 
					...
					<div class="radio-item">';
					    if( $color_semaforo=="#FFC500" ){
					        ...
					        echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="InicioA" name="droneA" checked value="#FFC500">';
					    }else{
					        echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="InicioA" name="droneA" value="#FFC500">';
					    }
					    echo '<label id="iniciolabel" for="InicioA"></label>
					</div>
					<div class="radio-item">';
					    if($color_semaforo=="#1CB707"){
					        echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="EnviadoA" name="droneA" checked value="#1CB707">';
					    }else{
					        echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="EnviadoA" name="droneA" value="#1CB707">';
					    }
					...
					<div class="radio-item">';
					if($color_semaforo=="#FF0C00"){
					    echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="PausadoA" name="droneA" checked value="#FF0C00">';
					}else{
					    echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="PausadoA" name="droneA" value="#FF0C00">';
					}
					...
					<div class="radio-item">';
					    if($color_semaforo=="#6900D1"){
					        echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="CerradoA" name="droneA" checked value="#6900D1">';
					    }else{
					        echo '<input onclick="act_semaforo('.$ap.',value)" type="radio" id="CerradoA" name="droneA" value="#6900D1">';
					    }
                            ...
                            if($mes1 == $mes2){
                                while($cont <= $dia2){
                                    if($n == 0 || $n == 6){
                                        ...
                                        if(empty($subject2) == false){
                                           // echo " entre al ciclo";
                                            $sql_fechaAnt = $mysqli -> query("SELECT fechaAnterior FROM piezas WHERE nombre = '$subject2' AND id = '$id'");
                                            ...
                                            $sql_fechaAnt = $mysqli -> query("SELECT fechaAnterior FROM odts WHERE proyecto = '$subject' AND id = '$id'");
                                            while($valor_fecha_anterior = mysqli_fetch_array($sql_fechaAnt)){
                                                $fecha_Ant = $valor_fecha_anterior[0];
                                                $fechaAnteriorOdt = substr($fecha_Ant, 8, 2);
                                                if($cont < 10){
                                ...
                                for ($i = 0; $i < $array_num; ++$i){
                                    ...
                                    while($cont <= $days_C){
                                        if($n == 0 || $n == 6){
                                            ...
                                            if(empty($subject2) == false){
                                                $sql_fechaAnt = $mysqli -> query("SELECT fechaAnterior FROM piezas WHERE nombre = '$subject2'");
                                                while($valor_fecha_anterior = mysqli_fetch_array($sql_fechaAnt)){
                                                    $fecha_Ant = $valor_fecha_anterior[0];
                                                ...
                                                $sql_fechaAnt = $mysqli -> query("SELECT fechaAnterior FROM odts WHERE proyecto = '$subject'");
                                                while($valor_fecha_anterior = mysqli_fetch_array($sql_fechaAnt)){
                                                    $fecha_Ant = $valor_fecha_anterior[0];
                                                    $fechaAnteriorOdt = substr($fecha_Ant, 5, 5);
                                                    $dia_anterior = substr($fecha_Ant, 8, 2);
                        ...
                        if($mes1 == $mes2){//CONDICION PARA CUANDO EL PERIODO SEA DENTRO DEL MISMO MES
                            ...
                            if (empty($subject2) == true){
                                ...
                                include_once 'tabla_piezas_un_mes.php';
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="apellido" id="apellido" class="form-control input-sm" required>
                        </div>
                        <div class="form-group">
                            <label>Fecha Salida</label>
                            <input type="date" name="email" id="email" class="form-control input-sm" required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="" id="telefono" class="form-control input-sm" required>
                        </div>
                        <div class="form-group">
                            <label>Área</label>
                            <select required class="form-control" style="height:60px; height: 40px;" name="areas_p" id="areas_p" value="" onchange="funcionAreas(this.value)">
                                <option value="" disabled selected>--Seleccionar--</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Persona Encargada</label>
                            <select required class="form-control" style="height:60px; height: 40px;" name="persona_enc" id="persona_enc" value="" onchange="funcionPersonas(this.value)">
                                <option value="" disabled selected>--Seleccionar--</option>
                            </select>
                            ...
                            <input type="text" name="fechaAnterior_Com_ODT" id="fechaAnterior_Com_ODT" class="form-control input-sm" hidden>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="modalPieza" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel_b" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form name='modalPiezaForm' action="../bd/save_piezasNew.php" method="POST" >
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                        ...
                        <div class="col">
                            ...
                            <input type="text" class="form-control"  id="nom_proyecto" name="nom_proyecto" class="nom_proyecto" value='<?php echo utf8_encode($subject); ?>' hidden>
                            <input type="text" class="form-control"  id="id_proyecto" name="id_proyecto" class="id_proyecto" value="<?php echo $id; ?>" hidden>
                            <input type="text" class="form-control"  id="tabla_tiempos" name="tabla_tiempos" class="tabla_tiempos" value=1 hidden>
                            <input type="text" class="form-control"  id="tabla" name="tabla" class="tabla" value="<?php echo $tabla; ?>" hidden>
                        </div>
                    </div>
                    <div class="row justify-content-center">
                        <div class="form-group">
                            <button type="button" class="btn btn-primary" value="Añadir Pieza"  onclick="valida_envia()" >Añadir Pieza</button>
...
<script>
    ...
    function valida_envia(){
        ...
        document.modalPiezaForm.submit();
    }
</script>
...
<script>
    ...
    var fechaFinalRango = fechaF.slice(0, 10);
    //alert(fechaFinalRango);
    $('#fechaInicio').attr('min', fechaInicialRango);
    $('#fechaInicio').attr('max', fechaFinalRango);
    $('#fechaSalida').attr('min', fechaInicialRango);
    $('#fechaSalida').attr('max', fechaFinalRango);
</script>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label>Fecha Inicio</label>
                            <input type="date" name="" id="apellidou" class="form-control input-sm" required>
                        </div>
                        <div class="form-group">
                            <label>Fecha Salida</label>
                            <input type="date" name="" id="emailu" class="form-control input-sm" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label>Área</label>
                                <select required class="form-control" style="height:60px; height: 40px;" name="areau" id="areau" value="" onchange="funcionAreasEditar(this.value)">
                                    <option value="" disabled selected>--Seleccionar--</option>
                                </select>
                        </div>
                        <div class="form-group">
                            <label>Persona Seleccionada</label>
                                <select required class="form-control" style="height:60px; height: 40px;" name="persona_enc_u" id="persona_enc_u" value="" onchange="funcion(this.value)">
                                    <option value="" disabled selected>--Seleccionar--</option>
                                </select>
                            <input type="text" name="" id="proyectou" class="form-control input-sm" hidden>
                            <input type="text" name="" id="piezau" class="form-control input-sm" hidden>
                            <input type="text" name="" id="fechau" class="form-control input-sm" hidden>
                            <input type="text" name="" id="fecha_Ant_ComODT" class="form-control input-sm" hidden>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel2">Agregar nuevo evento</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <label>Comentario</label>
                <input type="text" name="" id="comentarioO" class="form-control input-sm" required>
                <label>Color</label>
                <input type="color" id="color_eventO" class="form-control input-sm" required>
                <input type="text" id="dia_eventO" class="form-control input-sm" hidden>
                <input type="text" id="coment_asigO" class="form-control input-sm" hidden>
                <input type="text" id="proyectoeO" class="form-control input-sm" hidden>
                <input type="text" id="piezaeO" class="form-control input-sm" hidden>
            </div>
            <div class="modal-footer">
...
<div class="modal fade" id="modaleditarevento" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <label>Comentario</label>
                <input type="text" name="" id="comentario_u" class="form-control input-sm" required>
                <label>Color</label>
                <input type="color" id="color_event_u" class="form-control input-sm" required>
                <label>Fecha: </label>
                <small>Mes - Día</small>
                <input type="text" id="dia_event_u" class="form-control input-sm" required>
                <label>Motivo</label>
...
<script>
    $( document ).ready(function() {
        ...
        $("#guardarevento").click(function(){
            var color_fondo_tabla = $("#color_eventO").val();

            //alert(color_fondo_tabla);
            dia_event = $('#dia_eventO').val();
            //
            //$("td").css("background-color", color_fondo_tabla);
            document.getElementById(dia_event).style.backgroundColor = color_fondo_tabla;

            comentario = $('#comentarioO').val();
            color_event = $('#color_eventO').val();
            dia_event = $('#dia_eventO').val();
            coment_asig = $('#coment_asigO').val();
            proyecto_obt = $('#proyectoeO').val();
            pieza_obt = $('#piezaeO').val();

            const caracter = '&';
            if(proyecto_obt.includes(caracter)){
                console.log(`The word "${caracter}" is in the sentence`);
                proyectoe = proyecto_obt.replace(new RegExp(caracter,"g") ,"cod951");
                console.log(proyectoe);
            } else{
                proyectoe = proyecto_obt;
            }
            
            if(pieza_obt.includes(caracter)){
                console.log(`The word "${caracter}" is in the sentence`);
                piezae = pieza_obt.replace(new RegExp(caracter,"g") ,"cod951");
                console.log(piezae);
            } else{
                piezae = pieza_obt;
            }
...
//var boton = document.getElementById('guardar_evento');
    function borrar( cel ) {
        ...
        if(pieza_rec === ''){
            $("#proyectoeO").val(project);
            $("#piezaeO").val(project);
            $("#dia_eventO").val(id_td);
        }else{
            $("#proyectoeO").val(project);
            var pieza_rec = "<?php echo utf8_encode($Nombre_Pieza);?>";
            $("#piezaeO").val(pieza_rec);
            $("#dia_eventO").val(id_td);
        }
    };
</script>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#guardarnuevo').click(function () {
            ...
            proyecto_obt = $('#proyecto').val();
            pieza_obt = $('#pieza').val();
            fechaAnterior = $('#fechaAnterior_Com_ODT').val();
            
            const caracter = '&';
            if(proyecto_obt.includes(caracter)){
                console.log(`The word "${caracter}" is in the sentence`);
                proyecto = proyecto_obt.replace(new RegExp(caracter,"g") ,"cod951");
                console.log(proyecto);
            } else{
                proyecto = proyecto_obt;
            }
            
            if(pieza_obt.includes(caracter)){
                console.log(`The word "${caracter}" is in the sentence`);
                pieza = pieza_obt.replace(new RegExp(caracter,"g") ,"cod951");
                console.log(proyecto);
            } else{
                pieza = pieza_obt;
            }
            ...
            //alert(area);
            if( (apellido.trim() >= email.trim()) ){
                alertify.error("Fechas Incorrectas");
                $('#email').focus();
                return false;
            }else{
                agregardatos(nombre, apellido, email, telefono, area, persona, proyecto, pieza, fechaAnterior);
            }
        });
