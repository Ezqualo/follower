...
$sql_ids = "SELECT id FROM briefs WHERE nombre = '$subject'";
$resultos = mysqli_query($mysqli,$sql_ids);

while($ver_id = mysqli_fetch_row($resultos)){
    $id_piezas = $ver_id[0];
    //echo " id de cada pieza : " . $id;
}
...
if(empty($subject2) == false){
    ...
    if(isset($_SESSION['consulta'])){
        if($_SESSION['consulta'] > 0){
            ...
            $sql="SELECT * FROM piezasBrief where id='$idp' AND idPieza='$id'";
        }else{
            $sql="SELECT * FROM piezasBrief where idPieza='$id'";
        }
    }else{
        $sql="SELECT * FROM piezasBrief where idPieza='$id'";
    }
...
while($ver=mysqli_fetch_row($result)){
    ...
    <tr class="tt-wrapper">
        ...
        if ($useridRol == 1 || $useridRol == 2){
            ...
            <td>
                <button class="btn" onclick="preguntarSiNoPiezaBrief('<?php echo $ver[0] ?>')">
            ...
            <td>
                <button class="btn" onclick="preguntarSiNoPiezaBrief('<?php echo $ver[0] ?>')" disabled="true">
                    <img src="../img/delete.png" alt="" style="max-width: 20px; color:white;">
                </button>
            </td>
        ...
        for ($i = 0; $i < $array_num; ++$i){
            ...
            while($avanzar_contador <= $dias_contados2){ //INICIA CREACION DE CELDAS
                if($n == 0 || $n == 6){// CONDICION PARA SABER SI ES FIN DE SEMANA 
                    ...
                    //echo "--NO ENTRE AL IF ";
                    if ($fecha_cal_ini == $fecha_dia){//CONDICION PARA CUANDO EL EVENTO ES IGUAL A LA FECHA INICIAL 
                        //echo "--ENTRE AL IF ";
        ...
        echo '<td> 
					...
					<div class="radio-item">';
					    if( $semaforocomentario=="#FFC500" ){
					        ...
					        echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="InicioPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#FFC500">';
					    }else{
					        echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="InicioPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#FFC500">';
					    }
					    echo '<label id="iniciolabel" for="InicioPB'.$idSemcoment.'"></label>
					</div>
					<div class="radio-item">';
					    if($semaforocomentario=="#1CB707"){
					        echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="EnviadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#1CB707">';
					    }else{
					        echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="EnviadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#1CB707">';
					    }

					    echo '<label id="enviadolabel" for="EnviadoPB'.$idSemcoment.'"></label>
					</div>
					<div class="radio-item">';
					if($semaforocomentario=="#FF0C00"){
					    echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="PausadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#FF0C00">';
					}else{
					    echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="PausadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#FF0C00">';
					}
					
					echo '<label id="pausadolabel" for="PausadoPB'.$idSemcoment.'"></label>
					</div>
					<div class="radio-item">';
					    if($semaforocomentario=="#6900D1"){
					        echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="CerradoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#6900D1">';
					    }else{
					        echo '<input onclick="act_semaforo_pb('.$pb.',value)" type="radio" id="CerradoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#6900D1">';
					    }
					    
					    echo '<label id="cerradolabel" for="CerradoPB'.$idSemcoment.'"></label>
					</div>
		...
		echo '<td> 
					...
					<div class="radio-item">';
					    if( $semaforocomentario=="#FFC500" ){
					        ...
					        echo '<input  type="radio" id="InicioPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#FFC500" disabled>';
					    }else{
					        echo '<input type="radio" id="InicioPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#FFC500" disabled>';
					    }
					    echo '<label id="iniciolabel" for="InicioPB'.$idSemcoment.'"></label>
					</div>
					<div class="radio-item">';
					    if($semaforocomentario=="#1CB707"){
					        echo '<input  type="radio" id="EnviadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#1CB707" disabled>';
					    }else{
					        echo '<input type="radio" id="EnviadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#1CB707" disabled>';
					    }
					...
					<div class="radio-item">';
					if($semaforocomentario=="#FF0C00"){
					    echo '<input  type="radio" id="PausadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#FF0C00" disabled>';
					}else{
					    echo '<input  type="radio" id="PausadoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#FF0C00" disabled>';
					}
					...
					<div class="radio-item">';
					    if($semaforocomentario=="#6900D1"){
					        echo '<input  type="radio" id="CerradoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" checked value="#6900D1" disabled>';
					    }else{
					        echo '<input  type="radio" id="CerradoPB'.$idSemcoment.'" name="dronePB'.$idSemcoment.'" value="#6900D1" disabled>';
					    }
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col">
                        ...
                        <input type="text" class="form-control"  id="nom_proyectou" name="nom_proyectou" class="nom_proyecto" value='<?php echo $Nombre_Proyecto; ?>' hidden>
                        <input type="text" class="form-control"  id="tabla_tiemposu" name="tabla_tiemposu" class="tabla_tiempos" value=1 hidden>
                        <input type="text" class="form-control"  id="fechaAnt_PiezaBrief" name="fechaAnt_PiezaBrief" class="fechaAnt_PiezaBrief" hidden>
                    </div>
                </div>
            </div>
            <div class="modal-footer ">
                <button type="button" class="btn btn-warning" id="actualizadatosPiezaBrief" data-dismiss="modal">Actualizar
                </button>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#actualizadatosPiezaBrief').click(function () {
            //alert("Actualizado con etsito");
            //var Hola="Hola mundo";
            actualizadatosPiezaBrief();
        });
    });
</script>
